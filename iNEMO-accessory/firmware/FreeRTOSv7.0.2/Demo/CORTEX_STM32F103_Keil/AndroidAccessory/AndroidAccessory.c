...
{
    return usbNewInTransfer(1, androidAccessory.in, len, (char *)buff, nakLimit);
}

int androidAccessoryWrite(void *buff, int len)
{
    usbOutTransfer_NAK_LIMIT(1, androidAccessory.out, len, (char *)buff);
    return len;
}
